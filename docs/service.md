...
| Projects | `GET /api/projects`, `GET /api/projects/{project}/tables`, `.../runs`, `.../lineage`, `.../imports`, `.../imports/{id}` |
...
| Review and shipping | `GET /api/qa`, `GET /api/qa/image`, `POST /api/qa/status`, `POST /api/qa/comment`, `POST /api/qa/isolate`, `POST /api/qa/return`, `POST /api/qa/delete`, `POST /api/qa/ship` |
...
Example: ship a dataset once every image is reviewed.
...
`POST /api/training` starts `python -m granum.training.train` as a subprocess on the service's machine,
one job at a time. Progress, phase and log lines are available from `GET /api/jobs/{id}`, and cancelling
