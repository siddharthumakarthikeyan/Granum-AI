...
def _project(isolated_project, name="aerial"):
    images = [str(isolated_project.parent / f"img{i}.png") for i in range(3)]
...
def test_rename_rewrites_every_link(isolated_project):
    ...
    assert run.metrics_tables()[0].foreign_table_url.path.startswith(str(projects / "aerial-v2"))
    ...
    assert all(v.startswith(str(projects / "aerial-v2")) for v in holding.to_arrow().column("removed_from_version").to_pylist())
    # Review status and shipments follow.
    log = QaLog("aerial-v2", "data")
    assert log.current()[images[1]]["status"] == "reviewed"
    assert all(u.startswith(str(projects / "aerial-v2")) for u in log.shipped_urls())
    ...
    assert str(projects / "aerial2") + "/" in other
